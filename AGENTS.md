...
- 외부 API: 사용자 Flutter 앱(`/api/v1/**` 관리자 경로 제외 + `/oauth2/**`)과 별도 관리자 웹 프런트엔드(`/api/v1/admin/**`)가 같은 `qtai-server`를 호출한다. 관리자 UI는 Flutter 앱이 아니라 별도 웹이다. 이 결정은 2026-05-19 강사님 직강 기준이며, `03_아키텍처_정의서.md` v1.2 §4.9 / §13.6을 따른다.
...
- 작업 전 관련 문서와 기존 코드를 먼저 읽고, 저장소의 현재 구조와 관례를 우선한다.
- 구현 세부사항이 열려 있으면 기존 패키지 구조, 테스트 방식, 예외/응답 규칙을 따른다.
- 요구사항 변경이 필요해 보이면 임의로 바꾸지 말고 Lead 검토가 필요하다고 표시한다.
- 관련 없는 리팩터링, 대규모 포맷팅, 파일 이동, 메타데이터 변경은 하지 않는다.
- 사용자가 만든 변경사항을 되돌리지 않는다. 충돌하거나 작업에 직접 영향을 주는 경우에만 확인 후 진행한다.
- 보안상 민감한 값은 예시로도 작성하지 않는다.
...
2026-05-19 강사님 직강에서 확정한 도메인 표준 구조를 따른다. 기준은 `03_아키텍처_정의서.md` v1.1 §3.1이다.
...
저작권 표현은 안전을 단정하지 말고 "저작권 리스크를 낮춘다"를 사용한다. 이벤트 처리는 완전 무손실을 보장한다고 쓰지 말고 "핸들러 실패 로그와 재처리 가능 상태를 남긴다"로 표현한다.
