...
export default function ProtectedRoute() {
  const { adminError, adminInfo, adminLoading, isLoggedIn, logout, refreshAdminInfo } =
    useAuth();
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}>
        <Result
          ...
          extra={
            <Space>
              <Button type="primary" onClick={() => void refreshAdminInfo()}>
                다시 시도
              </Button>
              <Button onClick={logout}>로그인으로 이동</Button>
            </Space>
          }
        />
      </div>
