...
// - 단, 관리자 카카오 인증(POST /api/v1/admin/auth/**)은 service-user(8081, #452)에만 있고
//   admin-server(8090)엔 컨트롤러가 없다. 게이트웨이 확정 전까지 dev 한정으로 인증 경로만
//   VITE_AUTH_PROXY_TARGET(8081)으로 분리한다. 게이트웨이가 뜨면 두 값을 같은 주소로 두면 된다.
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // 관리자 인증은 service-user(8081) 소관 — 게이트웨이 확정 전 dev 전용 분리.
        '/api/v1/admin/auth': {
