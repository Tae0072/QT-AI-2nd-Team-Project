-- QT video playback metadata for service-bible.
-- Source videos are book-level full videos. Verse segments map each Bible verse
-- to a time range in the source video. qt_video_clips stores the final clip URL
-- and exposure status that the app can play for a QT passage.
