...
class AdminAiMonitoringQueryRepositoryTest {
    ...
    @Test
    @DisplayName("AD-08 응답은 assetStatuses 필드를 포함한다")
    void getAdminAiMonitoringReturnsAssetStatuses() {
        arrangeAssetStatusFixture();
        ...
        assertThat(response.assetStatuses().validating()).isEqualTo(1);
        assertThat(response.assetStatuses().approved()).isEqualTo(1);
        assertThat(response.assetStatuses().rejected()).isEqualTo(1);
        assertThat(response.assetStatuses().hidden()).isEqualTo(1);
    }

    private void arrangeAssetStatusFixture() {
