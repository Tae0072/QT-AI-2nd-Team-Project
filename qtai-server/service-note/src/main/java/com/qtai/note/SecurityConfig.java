 ...
 * service-note 보안 설정.
 *
 * <p>노트/나눔/신고 서비스를 무상태(STATELESS)로 운영하며, 컨트롤러의 메서드 보안
 ...
 *   <li>{@code /api/v1/admin/**} — denyAll. 관리자 기능(admin_role 이중검증 포함)은 이 서비스가 아니라
 *       admin-server가 제공한다. ROLE_ADMIN 단독 허용 시 admin_users.admin_role 검증 없이 열리는 우회를
 *       막기 위해 차단한다. 신고 검수도 admin-server 소관이라 여기서는 신고 "접수"만 노출한다.</li>
