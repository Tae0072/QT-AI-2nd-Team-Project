...
/**
 * {@link PurgeMemberSharingDataRestClientAdapter} 단위 테스트 — service-note sharing purge 호출 검증.
 */
class PurgeMemberSharingDataRestClientAdapterTest {
    ...
    void purge_정상() {
        ...
        var adapter = new PurgeMemberSharingDataRestClientAdapter(
                builder, endpoints, new SystemTokenProvider(TEST_SYSTEM_SECRET, 60_000L));

        server.expect(requestTo(startsWith(NOTE_BASE + "/api/v1/sharing/purge")))
                .andExpect(method(HttpMethod.POST))
                .andExpect(header("Authorization", startsWith("Bearer ")))
                .andRespond(withSuccess("{\"success\":true,\"data\":2}", MediaType.APPLICATION_JSON));

        assertThat(adapter.purgeByMemberId(7L)).isEqualTo(2);
        server.verify();
    }
}
