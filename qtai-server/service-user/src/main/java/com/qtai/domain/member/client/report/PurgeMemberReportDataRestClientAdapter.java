...
/**
 * report 도메인 {@link PurgeMemberReportDataUseCase}의 service-user 구현 — service-note HTTP 호출 어댑터.
 *
 * <p>보존기간 만료 회원 정리 배치(SYSTEM_BATCH)가 회원 접수 신고를 삭제할 때 쓴다. report는 service-note 소관이라
 ...
 */
@Component
public class PurgeMemberReportDataRestClientAdapter implements PurgeMemberReportDataUseCase {
    ...
    @Autowired
    public PurgeMemberReportDataRestClientAdapter(RestClient.Builder restClientBuilder,
    ...
    PurgeMemberReportDataRestClientAdapter(RestClient.Builder restClientBuilder,
                                           ServiceEndpointsProperties endpoints,
    ...
    public int purgeByMemberId(Long memberId) {
        ...
        try {
            ApiResponse<Integer> body = restClient.post()
                    .uri(uri -> uri.path("/api/v1/reports/purge").queryParam("memberId", memberId).build())
