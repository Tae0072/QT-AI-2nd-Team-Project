...
- `V30__create_notices.sql`이 PR diff에 포함되어 있고, `notices` 테이블은 `utf8mb4` charset으로 생성됨을 확인했다.
  - 재확인 명령: `git diff --name-status origin/dev-msa...HEAD -- qtai-server/admin-server/src/main/resources/db/migration`
  - 결과: `A qtai-server/admin-server/src/main/resources/db/migration/V30__create_notices.sql`
...
## REQUEST_CHANGES 6차 대응

- 단건 재시도 중 unique 제약 위반이 발생했더라도 동일 공지 알림 `eventKey`가 이미 존재하면 정상 멱등 skip으로 분류해 `failedCount`에 포함하지 않도록 변경했다.
- `NoticeService.preview` static helper를 `NoticeBodyPreview`로 분리하고, 목록 preview 길이와 알림 body 컬럼 한도를 상수화했다.
- fan-out 빈 회원 목록, 멱등 중복 skip, hide HIDDEN 재요청 실패, 잘못된 page/size, principal 변환 실패 테스트를 추가했다.
- OpenAPI AD-06 notice tag/path 설명을 한국어로 통일하고, 목록 `sort`가 `createdAt,desc` 단일 정렬임을 명시했다.
