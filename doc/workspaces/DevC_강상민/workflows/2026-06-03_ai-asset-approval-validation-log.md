...
| --- | --- |
| 담당자 | 강상민 |
| 브랜치 | `refactor/ai-asset-approval-validation-log` |
| PR 대상 | `dev` |
| 관련 F-ID | 해당 없음 |
...
| 구분 | 경로 | 책임 |
| --- | --- | --- |
| Modify | `qtai-server/src/main/java/com/qtai/domain/ai/api/dto/ReviewAiAssetCommand.java` | 승인 command 입력 계약에서 checklistVersionId 제거 |
...
1. `dev` 기준 `refactor/ai-asset-approval-validation-log` 브랜치에서 작업한다.
2. 테스트에서 `ReviewAiAssetCommand` 생성자와 approve 요청 body의 `checklistVersionId` 기대값을 먼저 제거한다.
3. `AiValidationLogRepository`에 `findFirstByAiAssetIdAndValidationLayerAndReviewerTypeOrderByCreatedAtDescIdDesc` 메서드를 추가한다.
