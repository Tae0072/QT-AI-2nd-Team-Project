...
> 작성자: DevC 강상민
> 기준일: 2026-06-09
> 목적: AI MSA 분리 이후 남은 작업, 선행 조건, 다음 작업 흐름을 팀이 바로 확인할 수 있게 정리한다.
...
| 영역 | 현재 상태 | 설명 |
| --- | --- | --- |
| AI boundary / contract | ✅ 완료 | AI 외부 의존, provider endpoint, fixture, smoke 준비가 고정됐다. |
...
| 🔒 조건부 | 최종 정리 | `ai-service-kafka-runbook` | Kafka 장애, replay, 재처리, idempotency 점검 runbook을 작성한다. | Kafka adapter/consumer 준비 | runbook 완료 | `docs/ai-service-kafka-runbook` |
...
- Kafka 구현은 `ai-event-outbox-relay-skeleton` 완료 후 시작한다.
- provider live 호출은 endpoint와 service auth 준비 후 실행한다.
- gateway route는 readiness, smoke, rollback 기준이 준비된 뒤 enable한다.
- monolith AI write path 제거는 ai-service cutover 안정화 후 진행한다.
- 문서에는 실제 환경 값이나 민감 값을 기록하지 않는다.
