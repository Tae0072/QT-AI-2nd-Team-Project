...
## 배경
- #343(Gradle 멀티모듈 + lib-common 추출)이 dev에 **squash 머지**됨. 본 작업은 그 후속(리뷰 권장사항 반영 + 인증 토대). squash로 기존 브랜치 충돌 → 최신 dev 기준 새 브랜치(`feature/msa-lib-common-jwt`)로 재구성.
...
## 범위
- 브랜치: `feature/msa-lib-common-jwt` (base: `dev`)
