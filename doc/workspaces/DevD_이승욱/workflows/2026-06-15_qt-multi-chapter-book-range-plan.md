...
### 4.1 스키마 — admin-server, 신규 `V46__qt_passages_multi_chapter_range.sql`
...
따라간다. **Flyway 마이그레이션은 admin-server에만** 추가(V46). `doc/admin-server-sync-rules.md` 준수.
...
| 스키마 | `admin-server …/db/migration/V46__qt_passages_multi_chapter_range.sql` (+ 2단계 정리 마이그레이션) |
...
- 롤백: V46는 컬럼 추가만이라 역호환. 코드 미배포 상태에서도 기존 컬럼으로 동작 유지.
...
2. admin-server V46 마이그레이션(컬럼 추가 + 백필).
