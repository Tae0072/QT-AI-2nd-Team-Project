...
### ② 스키마 (admin-server 단독, V46)
- `V46__qt_passages_multi_chapter_range.sql`: `end_book_id`/`end_chapter` 추가 + 기존 행 백필 + `NOT NULL` 승격 + FK.
  MySQL/H2 호환 위해 컬럼은 한 문장씩 NULL 추가 후 백필하고, 엔티티의 `nullable=false` 계약에 맞춰 `NOT NULL`로 승격한다.
- 자동 코드리뷰봇 diff는 `.sql`을 제외(성경 seed SQL 컨텍스트 폭발 방지)하므로, 봇의 스키마 정합성 검토를 위해 마이그레이션 전문을 아래에 그대로 노출한다. 실제 파일은 `qtai-server/admin-server/src/main/resources/db/migration/V46__qt_passages_multi_chapter_range.sql`.

```sql
-- V46__qt_passages_multi_chapter_range.sql (실제 마이그레이션 전문)
...
- Flyway 버전 순서: dev의 `V45__ensure_active_explanation_prompt`·`V47__convert_explanation_prompt_to_natural_instruction` 사이에 `V46`(우리) 위치 — 번호 충돌·공백 없음(V44→V45→V46→V47).
...
- **API 계약**: `04_API_명세서.md` §4.7.2에 `endChapter` 선택 필드, 기본값, 검증 규칙을 동기화했다.
- 실행 중 로컬 스택은 V46 미적용 DB와 호환 위해 기존 코드(00:02)로 service-bible 복구 배포함.
  교차 본문 라이브 수집은 머지 후 admin-server(V46)+service-bible 동시 배포 필요.

## 검증
- `./gradlew :service-bible:test :admin-server:test` 전부 BUILD SUCCESSFUL.
- 파서 단위 테스트: 같은 장 / 장 교차 정상 파싱, 종료 장 역전 거부.
- 양 모듈 import 테스트: 교차 장의 시작·종료 경계 절 포함 및 display order 검증.
- 양 모듈 mapper 테스트: 교차 범위 displayText(`9:20-10:5`) 검증.
- 관리자 서비스 테스트: 교차 장 정상 입력, 종료 장 역전, 같은 장 절 역전 검증.
- 일부 장 조회가 비면 기존 절 매핑을 교체하지 않고 startup 백필 재시도 대상으로 유지.
