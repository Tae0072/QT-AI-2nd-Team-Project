...
### ④ 리뷰 반영(REQUEST_CHANGES 대응, 2026-06-12)
- 백엔드 미리보기 로직을 정적·패키지 가시성 `NoteService.previewOf(...)`로 분리하고 `NoteBodyPreviewTest` 7케이스 추가(null/blank, body 우선, 섹션 폴백, 공백 정리, 한도 이하 그대로, truncate+말줄임표, 서로게이트 페어 보호) — "신규 백엔드 로직 단위 테스트 0건" 지적 해소.
- WARN 보강: 80자 경계가 서로게이트 페어(이모지 등)의 상위 코드면 한 글자 앞에서 잘라 깨진 글자가 남지 않게 처리.
