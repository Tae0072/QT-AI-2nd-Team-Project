...
`qt_passages`에 `end_chapter`/`end_book_id`를 추가(V46)했다. 관리자 QT 관리(admin REST + admin-web)도
...
| `admin-server …/db/migration/V46__qt_passages_multi_chapter_range.sql` | `end_book_id`/`end_chapter` 추가 + 백필 + `NOT NULL` 승격 + FK (admin-server 단독) |
...
- **배포**: 교차 본문 라이브 수집은 머지 후 admin-server(V46 적용)+service-bible 동시 배포 필요(현재 로컬 스택은 기존 코드로 복구).
