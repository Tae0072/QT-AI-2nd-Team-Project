...
| `service-gateway/.../support/JwtTestKeysContextCustomizerFactory.java` | (신규) 테스트 부팅 시 RSA 공개키 생성·주입(`gateway.jwt.public-key`) — 평문 키 미커밋, 모놀리식 패턴 동일 |
| `service-gateway/src/test/resources/META-INF/spring.factories` | (신규) 위 ContextCustomizerFactory 등록 |
...
- **검증 전용 책임분리**: 게이트웨이는 공개키만 보유, 발급(개인키)은 인증 서비스에만. 공개키는 환경변수 주입(저장소 평문 키 금지). 테스트도 평문 키를 커밋하지 않고 런타임 생성 키를 주입(모놀리식 `JwtTestKeys` 패턴, CLAUDE.md §8).
