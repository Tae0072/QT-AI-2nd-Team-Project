...
### (D) 공유 토큰 유출 대비 정책 (회전·범위)
공유 토큰이 신뢰 자격으로 격상되므로 유출 시 영향과 대응을 정의한다:
- **범위(scope)**: 토큰은 **bible-service 전용**(서비스별 개별 토큰, 단일 글로벌 토큰 금지). 게이트웨이는 라우트별로 해당 서비스 토큰만 주입. 유출 시 영향 범위를 한 서비스로 한정한다.
- **저장·주입**: env 주입만(`GATEWAY_BIBLE_SHARED_TOKEN`=`qtai.bible.gateway.shared-token`), 저장소 평문 금지(CLAUDE.md §8). 운영은 시크릿 매니저 권장.
- **회전(rotation)**: 정기 회전 + 유출 시 즉시 회전. 무중단 회전을 위해 **현재값+직전값 동시 허용(grace window)** 을 후속(Inc3b)에서 지원하도록 필터를 확장한다(현재는 단일값). 회전 절차는 게이트웨이·서비스 양측 env 갱신 → 점진 롤아웃.
- **노출 최소화**: 토큰 값은 로그·응답·감사 트레일에 남기지 않는다(필터는 SYSTEM 호출을 주체만 INFO 기록).
- **다층 방어 한계 명시**: 토큰은 네트워크/배포 경계 신뢰를 보완할 뿐이며, 서비스 메시·mTLS·네트워크 정책(게이트웨이/내부망만 접근)과 병행이 바람직(인프라 트랙).
