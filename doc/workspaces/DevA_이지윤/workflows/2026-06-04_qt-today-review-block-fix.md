...
수정 대상은 오늘 QT 범위 import, Flutter QT API 연동, 성서유니온 범위 파싱, 임시 더미 데이터 처리, 00:00/04:00 KST 노출 정책이다.
...
| 항목 | 원인 | 수정 방향 |
| --- | --- | --- |
| Flyway 더미 seed | 운영 migration에 임시 더미 seed가 들어가면 적용 후 삭제/수정 시 checksum 문제가 생긴다. | 운영 migration에서 제거하고 테스트/로컬 fixture로 이동한다. |
| 04:00 노출 정책 | 00:05 내부 import 후 캐시를 지우면 04:00 KST 수집 배치/사용자 노출 전 오늘 QT가 사용자에게 노출될 수 있다. | import 시 `todayQt` 캐시를 즉시 삭제하지 않고, 조회 로직에서 04:00 전에는 전일 본문을 반환한다. |
...
| 표준 예외 매핑 | 외부 호출/파싱/성경 책 이름 매칭 실패가 표준 런타임 예외였다. | `BusinessException`과 `ErrorCode`로 매핑한다. |
...
4. `QtTodayPassageImportService`에서 `@CacheEvict`를 제거해 00:05 KST 내부 import와 04:00 KST 수집 배치/사용자 노출 정책을 분리한다.
...
7. 외부 호출, 파싱 실패, DB 성경 책 이름 매칭 실패를 `BusinessException`으로 변환한다.
8. 관련 테스트와 문서를 수정한다.
...
- 운영 DB에 `V23__seed_dummy_today_qt_1co_verses.sql`이 이미 적용된 환경이 있으면 Flyway repair, baseline, 보상 migration 중 팀 운영 기준에 맞는 절차를 Lead와 확인한다.
