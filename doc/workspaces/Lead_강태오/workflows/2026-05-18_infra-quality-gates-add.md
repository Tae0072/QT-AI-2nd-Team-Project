...
## 추록 2 (2026-05-18 두 번째 CI 결과 — claude-review 진짜 원인 발견)

### 두 번째 CI 결과 (commit 16a44c3) — 6/7 success

- ci.yml 6 게이트 모두 success ✅ (수정 효과 확인)
- claude-review만 여전히 failure ❌

### claude-review 진짜 원인 (이미지 로그 확인)

- secret CLAUDE_CODE_OAUTH_TOKEN, ANTHROPIC_API_KEY 둘 다 설정되어 있음 ✅
- 진짜 원인: claude-pr-review.yml의 `PROMPT="..."` 안에 GitHub Actions context 변수를 직접 박아넣어서 발생한 bash syntax error
  - `${{ steps.get_diff.outputs.diff_content }}` 가 yaml 치환되어 PR diff 내용이 bash 스크립트 안에 raw text로 들어감
  - PR diff의 ci.yml 코드에 `count=$(find apis ...)` 같은 명령 substitution 패턴이 있어서 bash가 명령으로 해석하려다 line 127에서 syntax error
  - bash 큰따옴표 `"..."` 안에서도 `$(`, `\``, `$` 는 해석됨

### 세 번째 commit 변경 사항

- claude-pr-review.yml 수정: GitHub Actions context 변수를 env로 export 후 bash 변수($PR_DIFF, $DIFF_MAX_BYTES_INT)로 참조
- bash가 변수 한 번만 expand하고 내용 안 특수문자는 재해석 안 함 (표준 escape 패턴)

---

## 추록 3 (2026-05-18 세 번째 CI — claude-review self-deadlock 발견)

### 세 번째 CI 결과 (commit 11e5fb0) — bash syntax error 해결됨

- CI 6 게이트 모두 success ✅
- claude-review: bash syntax error 해결 → 실제로 실행되었으나 in_progress 상태 지속 (3분+)

### 진짜 원인: Auto Merge polling의 self-deadlock

claude-pr-review.yml의 actions/github-script@v7 코드:
```js
const THIS_JOB_NAME = 'Claude PR Auto Review';   // ← workflow name (X)
const ciChecks = check_runs.filter(r => r.name !== THIS_JOB_NAME);
```

GitHub Actions API의 check_runs는 각 check의 job name을 반환 (workflow name 아님).
실제 job name = "claude-review" (yaml의 jobs.claude-review).
따라서 filter에서 자기 자신이 빠지지 않음 → 자기 자신이 항상 in_progress → 영원히 polling → 10분 timeout.

### 네 번째 commit 변경 사항

- `THIS_JOB_NAME = 'Claude PR Auto Review'` → `'claude-review'` 로 수정
- 자기 자신을 ciChecks에서 정상 filter out
- 다른 6개 CI 결과만 확인 → 모두 success면 자동 머지 진행