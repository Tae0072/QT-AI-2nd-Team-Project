...
- **상태:** 응답 형태 **5개 합의 포인트 2026-06-10 FE 합의 완료**(§7). service-user task 3에서 구현 대기.
- **기준:** 기존 사용자 로그인(`POST /api/v1/auth/kakao`, `LoginResponse`)과 일관성 유지. 단일 DB·service-user가 유일한 JWT 발급자(RS256).

> ⚠️ **충돌 주의(엔드포인트 경로 한정):** 2026-06-09 전원 회신으로 확정된 결정 ③④는 **"기존 `/api/v1/auth/kakao` 재사용(신규 엔드포인트 없음)"** 이었다. 본 06-10 제안은 **신규 `POST /api/v1/admin/auth/kakao`** 를 신설한다 → 하루 전 확정과 충돌. 경로를 신규/재사용 중 어느 쪽으로 갈지는 **Lead·팀 재합의 필요**. 단, 본 계약의 **응답 형태 5개 포인트는 §7에서 FE 합의 완료**.
...
> 응답 형태 5개 합의 포인트는 아래와 같이 **FE 합의 완료**. (엔드포인트 경로 신규/재사용 자체는 별도 Lead 재합의 사안 — 위 '충돌 주의' 참고.)
...
| 0 | 엔드포인트 경로 | 신규 `admin/auth/kakao` | (응답 형태만 합의) | ⏸ 경로 자체는 06-09 확정과 충돌 → Lead 재합의 |
