...
1. **요청 필드**: `bookId`+`chapter`+`startVerse`+`endVerse` 기준 확정. ✅ **`04_API_명세서.md §4.7.2`를 본 PR에서 갱신**(요청 예시 status도 5종 `pending_review`로 정렬). §2상 **Lead(강태오) 명시 승인 코멘트 필요**. 별도 문서 저장소 캐논 04 동기화는 추가 필요.
