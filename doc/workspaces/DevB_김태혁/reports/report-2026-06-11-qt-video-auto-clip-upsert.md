# Report — 2026-06-11 qt-video-auto-clip-upsert
...
6. 리뷰 반영
   - 이벤트 경로 진입점에 `Propagation.REQUIRES_NEW` 적용
   - `QtPassageVerseMappingsChangedEvent`에 `eventId`, `eventType` 추가
   - 이벤트 처리 성공/실패 로그에 `eventId`, `eventType`, `handlerName`, `qtPassageId` 포함
   - 실패 로그에 `retryable=true`를 남겨 운영 재처리 판단에 활용 가능하게 함
   - import commit 이후 이벤트가 실제 clip row를 저장하는 통합 테스트 추가
   - 신규 `BibleVerseVideoSegmentRepository` JPQL을 `@DataJpaTest`로 검증
...
- `.\gradlew.bat :service-bible:test --tests "com.qtai.domain.qtvideo.internal.BibleVerseVideoSegmentRepositoryTest"` 통과
- `.\gradlew.bat :service-bible:test --tests "com.qtai.domain.qtvideo.internal.QtVideoClipPreparationEventIntegrationTest"` 통과
