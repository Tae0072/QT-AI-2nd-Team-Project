...
- `AFTER_COMMIT` 이벤트 핸들러에서 호출되는 clip 준비 진입점은 `Propagation.REQUIRES_NEW`로 분리한다.
- 이벤트에는 `eventId`, `eventType`, `qtPassageId`를 포함한다.
- 이벤트 실패 로그에는 `eventId`, `eventType`, `handlerName`, `qtPassageId`, `retryable=true`를 남겨 재처리 대상을 추적할 수 있게 한다.
- `importToday()` 커밋 이후 `@TransactionalEventListener(AFTER_COMMIT)`가 실제 `qt_video_clips` row를 저장하는 통합 테스트를 추가한다.
- `BibleVerseVideoSegmentRepository` JPQL은 `@DataJpaTest`로 검증한다.
...
- `BibleVerseVideoSegmentRepositoryTest`
  - active source video의 segment만 조회되는지 검증
- `QtVideoClipPreparationEventIntegrationTest`
  - import commit 이후 이벤트 경로로 `qt_video_clips`가 실제 저장되는지 검증
